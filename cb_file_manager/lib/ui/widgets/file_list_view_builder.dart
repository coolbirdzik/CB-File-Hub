...
class FileListViewBuilder {
  ...
  static Widget build({
    ...
    if (searchResults != null) {
      ...
      displayState = state.copyWith(folders: searchFolders, files: searchFiles);
    } else {
      displayState = state;
    }

    final effectiveMasonryLayout =
        isMasonryLayout ||
        (tabId != null &&
                    ...
                    child: LayoutBuilder(
                      builder: (context, constraints) {
                        ...
                        Widget buildGridItem(BuildContext context, int index) {
                          ...
                          final bool isSelected = selectionState.isPathSelected(
                            itemPath,
                          );
  ...
  static Widget _wrapWithPreviewPane({
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final double effectiveMinPreviewWidth = math.min(
          minPreviewWidth,
          maxPreviewWidth,
        );
...
class _PreviewPaneLayoutState extends State<_PreviewPaneLayout> {
  ...
  void _handlePreviewPointerDown(PointerDownEvent event) {
    ...
    // Runs after focus requests made for this same event have been applied,
    // so a child that claims focus on click (the video player) keeps it.
    scheduleMicrotask(() {
      if (!mounted || previewNode.hasFocus) return;
      previewNode.requestFocus();
    });
  }

  KeyEventResult _handlePreviewKeyEvent(FocusNode node, KeyEvent event) {
  ...
  Widget build(BuildContext context) {
    return ValueListenableBuilder<double>(
      ...
      builder: (context, currentWidth, child) {
        ...
        if (previewNode != null) {
          previewPane = Focus(
            focusNode: previewNode,
            onKeyEvent: _handlePreviewKeyEvent,
            child: Listener(
