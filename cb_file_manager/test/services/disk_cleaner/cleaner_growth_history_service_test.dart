...
String _snapshotKey(String drivePath) {
  ...
  final encoded = base64Url
      .encode(utf8.encode(withoutTrailingSlash))
      .replaceAll('=', '');
...
FullDiskScanResult _folderScan(
  ...
  return FullDiskScanResult(
    root: DiskTreeNode(
      name: r'C:\',
      fullPath: r'C:\',
      sizeBytes: children.fold<int>(0, (sum, child) => sum + child.sizeBytes),
      children: children,
    ),
...
void main() {
  ...
  test(
    'first completed scan creates a baseline without reporting growth',
    () async {
  ...
  test(
    'large snapshot traversal yields instead of monopolizing the caller',
    () async {
      final preferences = await SharedPreferences.getInstance();
      final service = CleanerGrowthHistoryService(preferences);
  ...
  test(
    'reports largest meaningful directory increases on the same drive',
    () async {
      final preferences = await SharedPreferences.getInstance();
      final service = CleanerGrowthHistoryService(preferences);
      final baselineAt = DateTime(2026, 8, 10, 9);
      await service.compareAndStore(
        _scan(usersBytes: 1024 * 1024 * 1024, cacheBytes: 100 * 1024 * 1024),
        scannedAt: baselineAt,
      );

      final comparison = await service.compareAndStore(
        _scan(usersBytes: 1400 * 1024 * 1024, cacheBytes: 300 * 1024 * 1024),
      ...
      expect(comparison.folders.first.increasedBytes, 376 * 1024 * 1024);
  ...
  test('does not compare folders overlapping incomplete coverage', () async {
    ...
    final comparison = await service.compareAndStore(
      _scan(usersBytes: 1400 * 1024 * 1024, cacheBytes: 500 * 1024 * 1024),
      scannedAt: DateTime(2026, 8, 12),
    );
  ...
  test(
    'persists bounded folder coverage flags instead of raw issue paths',
    () async {
      final preferences = await SharedPreferences.getInstance();
      final service = CleanerGrowthHistoryService(preferences);
      final folders = [
  ...
  test(
    'legacy coverage index preserves boundary and normalization semantics',
    () async {
      final preferences = await SharedPreferences.getInstance();
      final service = CleanerGrowthHistoryService(preferences);
  ...
  test(
    'legacy index handles thousands of issue paths without quadratic scans',
    () async {
      final preferences = await SharedPreferences.getInstance();
      final service = CleanerGrowthHistoryService(preferences);
      final folderPaths = [
  ...
  test(
    'suppresses an ancestor when a descendant explains the same growth',
    () async {
      final preferences = await SharedPreferences.getInstance();
      final service = CleanerGrowthHistoryService(preferences);
      await service.compareAndStore(
      ...
      final comparison = await service.compareAndStore(
        _scan(usersBytes: 1224 * 1024 * 1024, cacheBytes: 300 * 1024 * 1024),
        scannedAt: DateTime(2026, 8, 12),
      );
